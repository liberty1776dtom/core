...
Feature: Federation Sharing - sharing with users on other cloud storages
  ...
  @skipOnMICROSOFTEDGE
  Scenario: test resharing folder as readonly and set it as readonly by sharer
    Given using server "LOCAL"
    And these users have been created:
      | username |
      | user2 |
    ...
    And the user accepts the offered remote shares using the webUI
    And user "user1" from server "REMOTE" has shared "/simple-folder (2)" with user "user2" from server "LOCAL"
    And the user opens the share dialog for folder "simple-folder (2)"
    And the user sets the sharing permissions of "user2@%local_server_without_scheme%" for "simple-folder (2)" using the webUI to
      | delete | no |
    And using server "LOCAL"
    And user "user2" re-logs in to "%local_server%" using the webUI
    And the user accepts the offered remote shares using the webUI
